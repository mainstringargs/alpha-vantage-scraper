...
import io.github.mainstringargs.alphavantagescraper.output.timeseries.DailyAdjusted;
import io.github.mainstringargs.alphavantagescraper.output.timeseries.IntraDay;
import io.github.mainstringargs.alphavantagescraper.output.timeseries.Monthly;
import io.github.mainstringargs.alphavantagescraper.output.timeseries.MonthlyAdjusted;
import io.github.mainstringargs.alphavantagescraper.output.timeseries.Weekly;
import io.github.mainstringargs.alphavantagescraper.output.timeseries.WeeklyAdjusted;

/**
 * The Stock Time Series Data provides realtime and historical equity data in 4 different temporal
 * resolutions: (1) intraday, (2) daily, (3) weekly, and (4) monthly.
 */
public class TimeSeries {
    private final ApiConnector apiConnector;

    /**
     * Constructs a Time Series Data api endpoint with the help of an {@link ApiConnector} and a
     * {@link JsonParser}
     *
     ...
     * equity specified, updated realtime.
     *
    ...
    public IntraDay intraDay(String symbol, Interval interval, OutputSize outputSize) {
        String json = apiConnector.getRequest(new Symbol(symbol), Function.TIME_SERIES_INTRADAY,
                        interval, outputSize);
    ...
    /**
     * This API returns daily time series (date, daily open, daily high, daily low, daily close,
     * daily volume, daily adjusted close, and split/dividend events).
     *
     * @param symbol the stock symbol to lookup
     * @param outputSize the specification of the amount of returned data points {@link OutputSize}
    ...
    public DailyAdjusted dailyAdjusted(String symbol, OutputSize outputSize) {
        String json = apiConnector.getRequest(new Symbol(symbol),
                        Function.TIME_SERIES_DAILY_ADJUSTED, outputSize);
     ...
     * high, monthly low, monthly close, monthly volume).
     *
     * @param symbol the stock symbol to lookup
