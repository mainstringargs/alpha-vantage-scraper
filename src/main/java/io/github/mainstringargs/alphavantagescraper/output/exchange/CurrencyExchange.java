package io.github.mainstringargs.alphavantagescraper.output.exchange;
...
import io.github.mainstringargs.alphavantagescraper.output.exchange.data.CurrencyExchangeData;
    ...
    private static class CurrencyExchangeParser extends JsonParser<CurrencyExchange> {
        public CurrencyExchange resolve(JsonObject rootObject) {
            ...
            try {
                ...
                CurrencyExchangeData exchangeData = createCurrencyExchangeData(
                                data.values().stream().findFirst().orElse(Collections.emptyMap()));
        ...
        private CurrencyExchangeData createCurrencyExchangeData(Map<String, String> values)
                        throws JsonSyntaxException {
            return new CurrencyExchangeData(values.get("1. From_Currency Code"),
                            values.get("2. From_Currency Name"), values.get("3. To_Currency Code"),
                            values.get("4. To_Currency Name"),
                            Float.parseFloat(values.get("5. Exchange Rate")),
                            LocalDateTime.parse(values.get("6. Last Refreshed"),
