...
public class AVJSONReader {
  ...
  public static void main(String[] args) {
    ...
    for (StockData sd : stockData) {
      System.out.println(sd.getDateTime() + " " + sd.getOpen() + " "+sd.getLow() + " "+sd.getHigh() + " "+sd.getClose() + " "+sd.getAdjustedClose());
    }
  }
  ...
  public static List<StockData> getStockDataForDate(String ticker, LocalDate ldt) {
    ...
    if (directory.exists()) {
      ...
      for (File file : filesInDir) {
        ...
        if (fnTicker.equals(ticker) && (startDateTime.isBefore(ldt) || startDateTime.equals(ldt))
            && ((endDateTime.isAfter(ldt) || endDateTime.equals(ldt)))) {

          List<StockData> stockData = loadStockDataFromJSON(file);

          return stockData.stream().filter(sd -> sd.getDateTime().toLocalDate().equals(ldt))
