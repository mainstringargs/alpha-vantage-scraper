...
/**
 * Since the format for the time series responses differ slightly but on the whole have the same
 * structure the {@code TimeSeriesParser} extracts the similarity of the parsing to this class.
 *
 * @see JsonParser
...
public abstract class TimeSeriesParser<Data> extends JsonParser<Data> {
    ...
    /**
     * The specifics of the resolution is pushed down to each response type, i.e Intraday, Daily
     * etc.
     *
    ...
    /**
     * Gets the key for the stock data, this differs for each response type, i.e Intraday, Daily
     * etc. This is used by the resolve method below.
     *
    ...
    public Data resolve(JsonObject rootObject) {
        ...
        try {
            ...
            Map<String, Map<String, String>> stockData =
                            GSON.fromJson(rootObject.get(getStockDataKey()), dataType);
