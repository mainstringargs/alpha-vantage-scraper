package io.github.mainstringargs.output.sectorperformances;
...
import io.github.mainstringargs.output.sectorperformances.data.SectorData;
...
public class Sectors {
    ...
    Sectors(final Map<String, String> metadata, final List<SectorData> sectors) {
    ...
    /**
     * Percentual changes in different sectors during different timeperiods; from real-time to 10
     * years.
     *
    ...
    private static class SectorPerformancesParser extends JsonParser<Sectors> {
         ...
         */
        private SectorData createSectorData(String key, Map<String, String> values)
                        throws ParseException {
            return new SectorData(key,
                            Double.parseDouble(values.getOrDefault("Information Technology", "0")
                                            .trim().replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Health Care", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Consumer Staples", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Real Estate", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Materials", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Consumer Discretionary", "0")
                                            .trim().replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Energy", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Financials", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Industrials", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(values.getOrDefault("Utilities", "0").trim()
                                            .replace("%", "")),
                            Double.parseDouble(
                                            values.getOrDefault("Telecommunication Services", "0")
                                                            .trim().replace("%", "")));
        }
    }
}
