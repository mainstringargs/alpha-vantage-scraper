package io.github.mainstringargs.output;
...
/**
 * Every Data response from a top-level call, i.e intraday, daily, sector, etc, uses this class
 * internally to resolve the generic Data response. Since all Data responses have different
 * structures they need to implement the resolve logic.
 *
 * @param <Data>
 */
public abstract class JsonParser<Data> {
     ...
     */
    protected final DateTimeFormatter SIMPLE_DATE_FORMAT =
                    DateTimeFormatter.ofPattern("yyyy-MM-dd");
    protected final DateTimeFormatter DATE_WITH_TIME_FORMAT =
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
    protected final DateTimeFormatter DATE_WITH_SIMPLE_TIME_FORMAT =
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

    protected final String SIMPLE_DATE_PATTERN = "^\\d{4}-[01]\\d-[0-3]\\d";
    protected final String DATE_WITH_SIMPLE_TIME_PATTERN =
                    "^\\d{4}-[01]\\d-[0-3]\\d\\s+[0-2]\\d:[0-5]\\d:[0-5]\\d";
    protected final String DATE_WITH_TIME_PATTERN =
                    "^\\d{4}-[01]\\d-[0-3]\\d\\s+[0-2]\\d:[0-5]\\d:[0-5]\\d";
    ...
    /**
     * The logic the helper for the response need to implement in order to interpret the json
     * correctly.
     *
